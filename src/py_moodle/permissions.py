...
def get_user_role(session: requests.Session, base_url: str) -> str:
    ...
    The role is determined by checking the dashboard page for administrator
    markers. If the user has access to the site administration menu, the role
    is considered ``"admin"``; otherwise ``"user"``.
    ...
    role = "admin" if 'data-key="siteadminnode"' in resp.text else "user"
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if session is None or base_url is None:
                raise RoleError("Session and base_url are required for role verification.")
