    ...
    try:
        ...
        for i in range(NUM_COURSES_TO_CREATE):
            ...
            course = create_course(
                ...
                shortname=shortname,
                categoryid=1,
                numsections=1,  # We start with 1 thematic section
            )
            course_id = course["id"]
            created_course_ids.append(course_id)
            console.print(f"[green]✅ Course created with ID: {course_id}[/green]")

            # Get the ID of the first thematic section (position 1) to add content
            ...
            try:
                ...
                for filename in FILES_TO_UPLOAD_TO_FOLDER:
                    ...
                    if not file_path.exists():
                        ...
                        continue
                    add_file_to_folder(
                        session, base_url, sesskey, folder_cmid, str(file_path)
                    )
                    console.print(f"    - Added '{filename}'")
                    time.sleep(1)  # Give server a moment

                # 6c. Rename a file in the folder
            ...
            # 7. Show the final result of the course
            console.print(
        ...
        # --- Cleaning: Delete the courses created ---
